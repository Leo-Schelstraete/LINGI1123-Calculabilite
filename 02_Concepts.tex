...
\begin{itemize}
	\item Par \emph{un texte fini} déterminant sans contradiction ni ambiguïté le contenu 
		de la table.
	\item Par \emph{un algorithme}. Dans ce cas elle détermine la fonction \textit{ainsi} qu'un moyen de la calculer.
...
Ex : $f(x) = \left\{\begin{array}{ll} 1 & \text{s'il y a de la vie autre part que sur terre,} \\ 0 & \text{sinon.}\end{array}\right.$\\(en supposant qu'il n'y ai pas d'ambiguïté sur le terme ``vie'').\\
...
\label{exem:paire_entiers}
  L'ensemble des paires d'entiers, des triplets, \ldots
    \begin{proof}
        ...
        \newline
        \newline
Pour énumérer les paires d'entiers, on énumère les paires de somme 0, les paires de somme $1, \ldots$ De même pour énumérer les triplets, quadruplets, \dots 
...
\begin{myprop}
	...
	\begin{proof}
Soit $A$ un sous-ensemble infini (le cas fini étant trivial) d'un ensemble $E$ énumérable.  Comme $E$ est énumérable, il existe une énumération de ses éléments
\[
...
\begin{myprop}
 ...
 \begin{proof}
 Un langage est un ensemble de chaines finies de caractères d'un alphabet fini.  Il est donc  énumérable.
 \end{proof}
\end{myprop}
